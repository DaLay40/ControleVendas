...
namespace Utilitarios
{
    public class MyExeption : Exception
    {
        ...
        public MyExeption(string Mensagem, ETipoException Tipo = ETipoException.Erro, string Titulo = "Controle de Vendas")
        {
        ...
        public void OnMyException(object sender, ThreadExceptionEventArgs e)
        {
            if (e.Exception.Message.Contains("Exceção do tipo 'Utilitarios.MyExeption' foi acionada."))
            {
                ...
                if (ex.TipoException == ETipoException.Aviso)
                    MyMsgBox.Exibir(ex.Mensagem, ex.Title, EBotoesMyMsgBox.Ok, EIconeMyMsgBox.Aviso);

                if (ex.TipoException == ETipoException.Erro)
                    MyMsgBox.Exibir("Ocorreu um erro inesperado.", ex.Title, EBotoesMyMsgBox.Ok, EIconeMyMsgBox.Erro, ex);
            }
            else
            {
                ...
                MyMsgBox.Exibir("Ocorreu um erro inesperado.", "Controle de Vendas", EBotoesMyMsgBox.Ok, EIconeMyMsgBox.Erro, ex);
            }
        }
